            ...
            <div className="px-8 py-10 text-center">
              ...
              <div className="flex items-center justify-center mb-8">
                <div className="relative w-28 h-28">
                  ...
                  {/* Circle bg */}
                  <motion.div
                    className="absolute inset-4 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 shadow-lg shadow-emerald-300"
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                ...
                {step === 1 && (
                  ...
                  >
                    <div className="flex items-center justify-center gap-1.5 mb-3">
                      ...
                      <span className="text-xs font-bold text-amber-500 uppercase tracking-widest">Thành công!</span>
                      <Sparkles size={14} className="text-amber-400" />
                    </div>
                    ...
                    {/* Order ID chip */}
                    <div className="inline-flex items-center gap-2 bg-emerald-50 border border-emerald-200 text-emerald-700 px-5 py-2.5 rounded-full text-sm font-bold mb-6 shadow-sm">
                      <Leaf size={15} className="text-emerald-500" />
                      Mã đơn hàng: <span className="font-mono tracking-wider">{orderId}</span>
                    </div>

                    {/* Mini order summary */}
                    {cartItems.length > 0 && (
                      <div className="bg-slate-50 rounded-2xl border border-slate-100 px-4 py-3 mb-7 text-left space-y-2">
                        {cartItems.slice(0, 3).map((item) => (
                          <div key={item.id} className="flex items-center gap-3">
                            ...
                            />
                            <p className="flex-1 text-slate-700 text-xs font-medium line-clamp-1">{item.name}</p>
                            <span className="text-emerald-600 text-xs font-bold">×{item.quantity}</span>
                          </div>
                        ))}
                        {cartItems.length > 3 && (
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      <motion.button
                        ...
                        whileTap={{ scale: 0.97 }}
                        className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-2xl border-2 border-slate-200 text-slate-700 font-bold text-sm hover:bg-slate-50 hover:border-slate-300 transition-all"
                      >
                      ...
                      <motion.button
                        ...
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.97 }}
                        className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white rounded-2xl font-bold text-sm shadow-lg shadow-emerald-400/35 transition-all"
                      >
                        <PackageSearch size={16} /> Theo dõi đơn hàng
                      </motion.button>
                    </div>
